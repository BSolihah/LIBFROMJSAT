...
public class SeedSelectionMethods
{
    ...
    private static void kppSelection(final int[] indices, Random rand, final DataSet d, final int k, 
            final DistanceMetric dm, final List<Double> accelCache, boolean parallel)
    {
        /*
    ...
    static public void selectIntialPointsFFSSF(DataSet d, int[] indices, DistanceMetric dm, List<Double> accelCache, Random rand, boolean parallel, Set<Integer> selectedFeatures){
        int k= indices.length;
        ffSelectionSpesifiedFitur(indices, rand, d, k, dm, accelCache, parallel,selectedFeatures);
        
    }
    private static void ffSelectionSpesifiedFitur(final int[] indices, Random rand,final DataSet d, final int k,final DistanceMetric dm, final List<Double> accelCache, boolean parallel, Set<Integer> selectedFeature )
    {
        //Initial random point
        indices[0] = rand.nextInt(d.getSampleSize());
        ...
        final List<Vec> X = d.getDataVectors();
        int[] features = new int[selectedFeature.size()];
        int idxFeature =0;
        for(int idx:selectedFeature){
            features[idxFeature]= idx;
            idxFeature +=1;
        }
