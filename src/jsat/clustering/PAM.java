...
public class PAM implements KClusterer
{
     ...
     * @return the double 
     */
    protected double cluster(DataSet data, boolean doInit, int[] medioids, 
            int[] assignments, List<Double> cacheAccel, boolean parallel)
    {
    ...
    public double subCluster(DataSet data, boolean doInit, int clusters, 
            int[] assignments, List<Double> cacheAccel, boolean parallel,Set<Integer> selectedFeature)
    {
        ...
        if(doInit)
        {
            ...
            selectIntialPointsFFSSF(data,medoids,dm,accel,rand,false,selectedFeature);
        ...
        do
        {
            ...
            ParallelUtils.run(parallel, data.getSampleSize(), (start, end)->
            {
                ...
                for(int i = start; i < end; i++)
                {
                    if(i==features[counterSel]){
                        int assignment = 0;
                    double minDist = dm.dist(medoids[0], i, X, accel);

                    for (int k = 1; k < medoids.length; k++)
                    {
                        double dist = dm.dist(medoids[k], i, X, accel);
                        if (dist < minDist)
                        {
                            minDist = dist;
                            assignment = k;
            ...
            for(int i = 0; i < data.getSampleSize(); i++)
            {
                ...
                thisCandidateDistance = ParallelUtils.range(data.getSampleSize(), parallel)
                        ...
                        .mapToDouble(j -> Math.pow(dm.dist(medCandadate, j, X, accel), 2))
                        .sum();
                if(clusterID>=0 && clusterID< medoids.length){
                    if(thisCandidateDistance < bestMedCandDist[clusterID])
                {
                    bestMedCand[clusterID] = i;
                    bestMedCandDist[clusterID] = thisCandidateDistance;
            ...
            System.arraycopy(bestMedCand, 0, medoids, 0, medoids.length);
        }
        while( changes.sum() > 0 && iter++ < iterLimit);
