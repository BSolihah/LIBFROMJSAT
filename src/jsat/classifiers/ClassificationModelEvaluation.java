...
public class ClassificationModelEvaluation
{
    ...
    public void evaluateCrossValidationWithNonSeedRandom(int folds){
        evaluateCrossValidation(folds,RandomUtil.getNonSeedRandom());
    }
    ...
    private void evaluationWork(ClassificationDataSet firsttrainSet, ClassificationDataSet testSet, int index)
    {
       // ForOverlapPAMUndersampling forPAM = new ForOverlapPAMUndersampling();
        //ClassificationDataSet trainSet = forPAM.moveTheMajorityFromMix(firsttrainSet, 1000);
                
       // PAMBasedUndersampling pbu = new PAMBasedUndersampling();
        // ClassificationDataSet trainSet = pbu.SMOTEAndClusterbasedUndersampling(firsttrainSet);
      //  Sampling s = new Sampling();
        //KMeanBasedUndersampling kmean=KMeanBasedUndersampling();
       // ClassificationDataSet trainSet = s.underSampling(firsttrainSet);
       // ClassificationDataSet trainSet = s.clusterBasedSampling(firsttrainSet);
      // ClassificationDataSet trainSet = s.SBCSampling(firsttrainSet);
       // ClassificationDataSet trainSet =s.SBCAndMajClusterSampling(firsttrainSet);
     //  ClassificationDataSet trainSet = s.majClusterBasedUndersampling(firsttrainSet);
     // ClassificationDataSet trainSet = s.majkmeanBasedUndersampling(firsttrainSet);
     ...
     // ClassificationDataSet trainSet= hdbU.clusterDBScanSMOTERandom(dataSet, 2);
      //   ClassificationDataSet trainSet= hdbU.clusterDBScanSMOTERandom(dataSet, 1);
      //  ClassificationDataSet trainSet= hdbU.clusterDBScan(dataSet, 5, 40, 2);
        ClassificationDataSet trainSet = firsttrainSet;
    ...
    public double[][] getConfusionMatrix()
    {
     ...
     /*
    additional method for performance measure
    
    Sensitivity: TP/(TP+FN)
    Specificity: TN/(FP+TN)
    Precission:TP/(TP+FP)
    Recall: TP/(TP+FN)
    Gmean1= (sensitivityxspecificity)
    Gmean2=(sensitivityxprecission)
    AdjustedGeometric Mean :Agm
    AGm = (G − mean + specificity × Nn )/(1 + Nn) , ifspecificity > 0 (8.11)
    AGm = 0, ifsensitivity = 0

    */
    public double getSensitivityMeasure(int mode){
        double s = 0;
        double[][] cm = this.getConfusionMatrix();
        if(mode == 2){
            s = cm[0][0]/(cm[0][0]+cm[1][0]);
        }
        return s;
    }
    public double getSpecificityMeasure(int mode){
        double s = 0;
        double[][] cm = this.getConfusionMatrix();
        if(mode == 2){
            s = cm[1][1]/(cm[1][0]+cm[1][1]);
        }
        return s;
    }
    
    /**
     * Assuming that we are on the start of a new line, 
     * the confusion matrix will be pretty printed to {@link System#out System.out}
     */
